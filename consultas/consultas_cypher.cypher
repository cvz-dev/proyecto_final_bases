...
MATCH (i:Instructor)-[:BELONGS_TO]->(d:Department)
WITH d, avg(toFloat(i.salary)) AS promedio_depto
MATCH (i:Instructor)-[:BELONGS_TO]->(d)
...
MATCH (d:Department)<-[:BELONGS_TO]-(i:Instructor)
...
MATCH (d:Department)
WHERE NOT (:Instructor)-[:BELONGS_TO]->(d)
...
MATCH (sec:Section)-[:OF]->(c:Course),
      (sec)-[:HELD_IN]->(cl:Classroom)
WHERE sec.year = 2009
...
MATCH (s:Student)-[t:TAKES]->(sec:Section)-[:OF]->(c:Course)
...
MATCH (c:Course)
WHERE NOT EXISTS {
    MATCH (c)<-[:OF]-(sec:Section)<-[:TAKES]-(:Student)
}
